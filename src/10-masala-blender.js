...
export function pipe(...fns) {
  ...
  return function(initial){
    return fns.reduce((currResult, fn) => fn(currResult), initial)
  }
}
...
export function compose(...fns) {
  ...
  return function(initial){
    return fns.reduceRight((currResult, fn) => fn(currResult), initial)
  }
}
...
export function createRecipe(steps) {
  ...
  const actualFunctions = steps.map((step) => map[step])
  const validFunctions = actualFunctions.filter((fn) => fn !== undefined)

  return pipe(...validFunctions)
}
